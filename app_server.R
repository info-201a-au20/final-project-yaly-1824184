...
server <- function(input, output) {
  ...
  output$bar <- renderPlot({
    ...
    p <- p + geom_bar(
      mapping = aes_string(x = "genre", fill = input$bar_fill),
      position = "stack"
    ) +
  ...
  output$genre_plot <- renderPlotly({
    movies_genre_edit_select <- filter(
      movies_genre_edit, genre %in% input$genre
    )
    fig <- plot_ly(
      data = movies_genre_edit_select,
      x = ~genre,
      y = ~inflation_adjusted_gross,
      type = "box"
    ) %>%
      layout(
        title = "Amount Grossed Compared by Genre",
        yaxis = list(
          range = c(0, input$genre_range),
  ...
  output$scatter_plot <- renderPlotly({
    ...
    plot2 <- plot_ly(
      ...
      layout(
        title = "The Gross of Disney Movies throughout the Year",
        xaxis = list(
          title = "Years"
        ),
        yaxis = list(
