...
page_one <- tabPanel(
  ...
  tags$body(
    mainPanel(
      h1("Disney Movies"),
      p(
        "Our domain of interest is entertainment so in this report,
               ...
               This", a(
          href = "https://www.kaggle.com/prateekmaj21/disney-movies",
          "data set"
        ),
      ...
      img(
        src = "Walt-Disney-Pictures-Logo.jpg", width = "750px",
        height = "430px"
      ),
      ...
      tags$ol(
        ...
        tags$li("How does the genre of a", tags$b("Disney"), "movie affect the
                amount of gross income the film will produce?"),
...
rating_pick <- checkboxGroupInput(
  inputId = "rating_pick",
  label = "Select Rating",
...
page_two <-
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        p("This scatter plot is an overview of", tags$b("Disney"), "movies
        between 1937 to 2016. On the left hand side, change the settings you
        want the graph to filter by.You may select one genre in the dropdown
        menu and select up to 5 ratings. This will enable you to view which
...
genre_plot_sidebar_content <- sidebarPanel(
  ...
  radioButtons(
    "genre_range", "Inflation adjusted gross",
    list(
...
genre_plot_main_content <- mainPanel(
  p(
    "This chart compares the inflation adjusted gross of", tags$b("Disney"),
...
page_four <- tabPanel(
  ...
  sidebarLayout(
    ...
    mainPanel(
      p(
        "This chart compares the highest grossing", tags$b("Disney"),
        "films of all time. On the left hand side, choose if you want to compare
        the top 10 or 20 movies than see the genre and ratings of these films.
        We can study which types of movies are most succesful. You can also
        study which years these top movies were made."
      ),
      plotOutput("bar")
    )
  )
...
page_five <- tabPanel(
  ...
  mainPanel(
    ...
    p("Overall, after the 1990s, Disney tended to have produced more movies
      ...
      each genre. However, in 1937, the movie ‘Snow White and the Seven
      Dwarfs’ significantly skewed the plot in the musical genre because
      its inflation adjusted gross was the largest of over 5.2 billion
      dollars. Snow White was Disney’s first movie, which suggests that
      this movie must have continued to accumulate its gross earnings since
      then. Also, Snow White is a rated G (general audience) movie, which
      is Disney’s popular theme."),
    h1("2nd Takeaway:"),
    ...
    p("Sometimes, a small subset of a dataset can tell a different story than
      ...
      most success might shift from G rated movies to PG-13 ones in coming
      years. To see the data for yourself, and to compare it with the top 10
      Disney films, you can navigate to the 'Top Grossing Films' page. Disney
      might need to continue focusing a bit more on their older audience rather
      than younger since that has really been working for them recently.
      Studying the top 20 movies and recent trends is a good way to understand
      Disney's audience and what they like. The better Disney understands their
      audience, the better they can cater to them. For example, if Disney knows
      their audience loves adventure films, they will continue making more. That
      way, they make more money and the audience is happy too."),
  )
)
