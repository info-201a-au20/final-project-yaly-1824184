...
this data the highest gross movie is `r Highest_Gross_Movie`. Also, the lowest 
gross movies would be the movies `r Lowest_Gross_Movies`. With this being said, 
the most common genre from the data are `r Most_Common_Genre` and the most 
common release date year would be the `r Most_Common_Release_Year`. Finally, 
in this data it displays `r Total_Movies`. This can all be seen below.
```{r}
summary
```
...
This chart plots the relationship between the inflation adjusted gross over all the different genres of movies in the dataset. We made this box plot to see which genres grossed the most money for Disney.
